...
let _cache=null
export function getAllVoices(){ try{ return window.speechSynthesis.getVoices()||[] }catch(e){ return [] } }
export function refreshVoices(){ _cache=null; return [] }
...
function vwGuess(name,scene){ const n=(name||'').toLowerCase(); if(scene==='kid')return /\b(xiaoshuang|child|kid|baby)\b|儿童|小孩/.test(n); if(scene==='uncle'||scene==='newsm')return /\b(male|yunyang|yunjian)\b|男/.test(n); return /natural|online|neural/.test(n) }

// 计算该场景的目标音调（若用户在 speak 传 pitch 则覆盖）
function scenePitch(scene){ const s=SCENES.find(x=>x.id===scene); return s&&s.pitch||0.98 }
