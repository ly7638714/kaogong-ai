...
// 计时状态已抽到共享 composable（useExamTimer），萌宠悬浮条也能控制开关、展示实时倒计时
const { runSec, countQuestions, startStopwatch, stopStopwatch, assessTime, fmtSec } = useExamTimer()
...
async function drawTutuAnno(msg, q) {
  ...
  try {
    ...
    if (svg && svg.includes('<svg')) {
      ...
      showToast('⚠️ 标注图生成失败（模型未输出有效 SVG；可在出卷参数填「出题快模型」如 deepseek-flash 提速画图）', 'error')
    }
  } catch (e) {
...
function speakWithScript(txt, onEnd, trackMessage = false, skipRewrite = false, onError = null, question = '', cacheOnly = false, pinCache = false, baseOverride = null) {
  ...
  const playReady = (ready) => {
    ...
    const endCb = base.onEnd
    base.onEnd = () => { if (typeof endCb === 'function') endCb(ready) }
    speak(ready, base)
  }
...
async function toggleSpeak(ev) {
  ...
  if (m && m._ttsCached && Array.isArray(m._ttsSegments) && m._ttsSegments.length) {
