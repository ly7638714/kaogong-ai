...
<template>
  <div v-if="show" class="ov show" @click.self="closeHub()">
    <div class="pnl" style="max-width: 560px">
      <h3>🗓️ 今日复习中枢</h3>
      <p style="font-size: calc(12px * var(--ui-fs-scale, 1)); color: var(--text3); margin: 2px 0 10px">到期 = 按记忆曲线该回访；📖 学新 = 挑没学过的词条先记住（按 1/2/4/7/15/30 天排期）。</p>
      ...
      <template v-else>
        <div style="display: flex; gap: 6px; margin-bottom: 8px">
          <button class="btn" :class="memMode === 'due' ? 'btn-pri' : 'btn-gh'" style="padding: 3px 10px; font-size: calc(12px * var(--ui-fs-scale, 1))" @click="memMode = 'due'">🔁 到期（{{ memDue.length }}）</button>
          <button class="btn" :class="memMode === 'new' ? 'btn-pri' : 'btn-gh'" style="padding: 3px 10px; font-size: calc(12px * var(--ui-fs-scale, 1))" @click="memMode = 'new'">📖 学新词条</button>
        </div>
        ...
        <template v-if="memMode === 'due'">
          ...
          <div v-for="(item, i) in memDue" :key="item.key" class="focus-item">
            ...
            <div class="fi-body">
              ...
              <div v-if="infoOf(item.title)" class="fi-q" style="color:var(--text3);font-size: calc(12px * var(--ui-fs-scale, 1));margin-top:2px">📖 {{ infoOf(item.title).slice(0, 140) }}</div>
            </div>
        ...
        <template v-else>
          <div style="display: flex; gap: 6px; margin-bottom: 8px; flex-wrap: wrap">
            <button v-for="c in MEM_CATS" :key="c" class="btn" :class="memCat === c ? 'btn-pri' : 'btn-gh'" style="padding: 2px 10px; font-size: calc(12px * var(--ui-fs-scale, 1))" @click="switchMemCat(c)">{{ c }}</button>
          </div>
          ...
          <div v-for="(it, i) in fresh" :key="it.key" class="focus-item">
            ...
            <div class="fi-body">
              ...
              <div v-if="extraText(it)" class="fi-q" style="color:var(--text3);font-size: calc(12px * var(--ui-fs-scale, 1));margin-top:2px">📖 {{ extraText(it).slice(0, 160) }}</div>
            </div>
