...
export const APP_FEATURES = [
  ...
  { id: 'sync_tab', name: '数据同步与保存', entry: '顶部导航「💾 数据同步」', desc: '集中处理本地文件夹保存、自动备份、Gitee/GitHub/WebDAV 云同步，以及导出和导入备份', how: '点击顶部「💾 数据同步」，设置面板会直接展开“数据与同步”分组', tips: '换设备时两端使用同一种同步方案；重要数据也建议定期导出 JSON 备份' },
  { id: 'export_data', name: '导出', entry: '顶栏「📤 导出」', desc: '导出学习数据/错题/对话记录等', how: '点导出按引导操作', tips: '可用于备份或打印' },
  { id: 'set_models', name: '设置·模型', entry: '顶部导航「⚙️ 设置」→ 模型与语音', desc: '配置文字模型（DeepSeek 等）、视觉模型（deepseek-v4-flash-vision-exp 等）、对话快模型（deepseek-v4-flash 非思考）、图形增强模型', how: '填对应 Key 与模型名；图形增强支持硅基流动/Ollama', tips: '视觉模型用于发图题；快模型用于「⚡快答」秒回' },
