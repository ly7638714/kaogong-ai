...
const ZL_ROTATION = ['基期/现期', '增长率', '增长量', '比重', '平均数', '倍数', '隔年增长', '年均增长', '混合增长率', '拉动增长/贡献率']
const MAT_TYPES = {
  text: '纯文字材料（统计公报式段落，含时间/范围/单位）',
  table: '表格材料（Markdown 表格，含表头/单位/时间口径，多行多年数据）',
  mixed: '混合材料（文字背景+表格数据）',
  chart: '图形数据材料（柱状/折线/饼图类：必须在材料区输出 [ECHARTS] {ECharts option JSON，含 xAxis/yAxis 或 series.data，series.type 用 bar/line/pie，标题/颜色清晰} [/ECHARTS] 供前端渲染成精美交互统计图；同时输出 Markdown 数据表格兜底，图表与表格数据完全一致、可验算）'
}
export function buildMaterialPrompt(opts = {}) {
  ...
  return (
    ...
    '2. 题组结构（严格按此，考点递进）：\n' + qs + '\n' +
    '3. 输出格式（严格）：\n### 📄 材料\n（材料内容；表格一律用 Markdown 表格；若材料是图形数据材料，材料区先输出 [ECHARTS] {ECharts option JSON} [/ECHARTS] 供前端渲染，紧跟 Markdown 数据表格，二者数据完全一致）\n### 第1题\n（题干，承接材料，无歧义）\nA. … B. … C. … D. …\n【正确答案】X\n### ✅ 解析\n（正确项分析 + 每个干扰项错因点名；本次为提速，解析可省略不输出，稍后按需单独生成）\n…（依此类推到第' + n + '题）'
  )
}
...
export function buildQuizSys(opts = {}) {
  ...
  const mainFormat = plate === '类比推理'
    ? '只输出：题干（直接给二词、三词、成语、古诗或填空式词项结构，绝不输出任何附加问法行）+ 4 个选项（A. … B. … C. … D. …）+ 单独一行【正确答案】X。\n'
