...
export const APP_FEATURES = [
  { id: 'home_tabs', name: '主页六大入口', entry: '顶部导航：💬对话 / 📚知识库 / 🗂积累 / 📊统计 / 📋错题 / 🚀看板 / 💾数据同步 / ⚙️设置', desc: '项目所有主功能从顶部导航进入', how: '点对应按钮切换到该页面', tips: '当前所在页面可用导航高亮判断' },
  { id: 'chat_train', name: '训练中心', entry: '💬对话页 · 训练工具「🎯 训练中心」', desc: '统一入口：进入后按需选择训练模块，再进入对应配置页', how: '点「🎯 训练中心」→ 从模块卡选择 单题快练/AI 整卷出题/导入材料/错题集组卷/真题快练/晨练包/每周重做 → 按需配置后开始', tips: '离线练习、锚点自测、立体图推、资料速算等仍在训练工具“快捷工具”分组；网页含“真题快练”' },
  { id: 'chat_single', name: '单题快练', entry: '💬对话页 · 训练中心「⚡ 单题快练」', desc: '选板块随机出 1 题，即时批改、可再来一题、错题入库', how: '先选板块（判断推理/言语/图形/资料分析/数量/常识…），进「单题快练」→ 作答 → 看解析；可点「再来一题」', tips: '资料分析支持自定义材料形式（纯文字/表格/统计图/综合）与单题或一篇5题；AI 出题会经本地质检保证唯一正确项' },
  { id: 'chat_paper', name: 'AI 整卷出题', entry: '💬对话页 · 训练中心「🎲 AI 整卷出题」', desc: '按国考/省考卷面模板 AI 组卷，支持导入材料识别、错题组卷、限时作答批改', how: '选模板（或自定义）→ AI 按卷面结构出题（资料分析按一篇材料5题）→ 限时作答 → 自动批改', tips: '可用错题本一键组卷；材料导入支持文字/表格/图表识别' },
  ...
  { id: 'chat_timer', name: '考场计时', entry: '🐾萌宠悬浮条「⏲ 计时」（对话页右上角萌宠旁）', desc: '按问数限时（1问=1分钟），AI 回复后统计用时', how: '点萌宠悬浮条的「⏲ 计时」即可开启/关闭；开启后每发一题按问数限时，悬浮条实时显示剩余时间，超时标红「超时」', tips: '适合模拟考场节奏；计时开关已迁入萌宠常用悬浮，对话输入区更清爽' },
  ...
  { id: 'chat_read', name: '消息朗读', entry: '回复消息操作栏「🔊 朗读」+ 全局朗读按钮', desc: '用真人级 TTS 朗读回复/题干/错题，支持暂停、继续、倍速', how: '点「🔊」朗读该条，或用顶栏 🔊 读当前页面；⏸ 暂停、▶ 继续、⏱ 切换倍速、⏹ 停止', tips: '支持 0.75-1.5 倍速；语音设置里可换音色/克隆声线；开头「嘟」声默认已智能裁掉，个别音色仍能听到可设强制裁剪毫秒数' },
  ...
  { id: 'stats_tab', name: '统计', entry: '顶部导航「📊 统计」', desc: '做题/正确率/板块掌握度等学习数据', how: '进入统计页查看各项学习数据', tips: '看板是可视化总览版；周报/月报还带复盘率、消化率与复错率' },
  { id: 'ck_tab', name: '看板', entry: '顶部导航「🚀 看板」', desc: '学习驾驶舱：各板块掌握度、励志语录、阅读/语音等', how: '点开查看与交互', tips: '含各板块细分掌握度与激励内容' },
  { id: 'sync_tab', name: '数据同步与保存', entry: '顶部导航「💾 数据同步」', desc: '集中处理本地文件夹保存、自动备份、Gitee/GitHub/WebDAV 云同步，以及导出和导入备份', how: '点击顶部「💾 数据同步」，设置面板会直接展开“数据与同步”分组；同步已拆成「⬆️ 上传本机备份 / ⬇️ 下载云端最新 / 🔀 智能合并」三个独立按钮', tips: '换设备时两端用同一种方案，然后点「🔀 智能合并」最安全；API Key 与密码在备份/同步前会自动打码，不会进云端文件；重要数据也建议定期导出 JSON 兜底' },
  { id: 'export_data', name: '导出', entry: '顶栏「📤 导出」', desc: '导出学习数据/错题/对话记录等', how: '点导出按引导操作', tips: '可用于备份或打印' },
  { id: 'set_models', name: '设置·模型', entry: '顶部导航「⚙️ 设置」→ 🧠 模型', desc: '配置文字模型（DeepSeek 等）、视觉模型（DeepSeek 用同一个 Key、模型填 deepseek-flash 即可识图）、对话快模型（deepseek-flash 等极速档）、图形增强模型、朗读讲稿改写模型', how: '填对应 Key 与模型名；每张卡片都有「🧪 测试连通性」；图形增强支持硅基流动/Ollama', tips: '视觉模型用于发图题；快模型用于「⚡快答」秒回；讲稿改写模型可让朗读更像讲课' },
  { id: 'set_tts', name: '设置·语音音色市场', entry: '⚙️ 设置 → 🗣️ 语音', desc: '五路引擎：智谱 GLM-TTS（超拟人·收费）/ 阿里百炼 Qwen3-TTS / OpenAI 兼容（含 🆓 一键接入本机免费开源 CosyVoice2·GPT-SoVITS，可克隆、0 费用）/ Edge 免费神经 / 系统语音兜底；支持音色试听、改名、删除、自定义克隆', how: '选引擎 → 试听 → 用「+ 自定义」上传 3-30 秒音频克隆声线；想彻底免费就点「🆓 一键接入本机免费开源 TTS」', tips: 'Edge 免费神经在部分网络会被拦截（试听失败就是被拦了），此时改用系统语音或自建开源引擎；命中本地缓存的朗读不再计费' },
  { id: 'set_voice_pause', name: '朗读暂停 / 继续 / 倍速', entry: '萌宠悬浮条 与 萌宠面板 的 ⏸ / ▶ / ⏱ / ⏹', desc: '朗读可随时暂停、继续、切倍速、停止', how: '点 ⏸ 暂停（冻结整条时间轴）→ 点 ▶ 从原位置无缝接着读，不重头、不重新请求、不再计费；⏱ 循环切倍速；⏹ 停止', tips: '暂停后改倍速再继续，会按新倍速从本段开头重读，保证倍速真实生效' },
  { id: 'set_voice_bind', name: '角色绑定已有克隆声线', entry: '⚙️ 设置 → 🎵 趣味与陪伴 → 萌宠角色', desc: '内置锁定角色可直接绑定你「已经克隆过」的音色，不必重复克隆花钱', how: '在角色处点「🔗 绑定已有克隆声线」→ 从已有音色里挑一个 → 支持一键解绑、恢复该角色内置声线', tips: '内置角色自身不会再触发自动克隆，避免反复消耗 Key' },
...
export function petDetectUi() {
  ...
  if (!parts.length) {
    ...
    else if (t === 'sync') parts.push('💾数据同步')
