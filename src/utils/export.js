...
export function getPayload(type){
 ...
 const cItems=[],cParts=[]; c.forEach(it=>{ if(it.role==='user'){ const bk=(it.text||'').match(/判断推理|言语理解|图形推理|资料分析|数量关系/)?'':'' } cItems.push({type:'msg',role:it.role,text:stripMd(it.text),imgs:it.imgs}); cParts.push(((it.role==='user'?'【我】':'【AI】')+it.text)) })
...
async function exportMdDocx(title, md){
 ...
 try{ const blob=await buildDocx({title,paragraphs}); downloadBlob(blob,title+'.docx') }catch(e){ downloadText(pdfHtml(title,[{type:'msg',role:'ai',text:md}]), title+'.doc','application/msword') }
}
